const API = '';
let allProducts = []; // This will store our data locally
...
async function loadProducts() {
  ...
  try {
    ...
    allProducts = await res.json(); // Save data to global variable
    spinner.className = 'spinner';

    displayProducts(allProducts); // Use our new display helper
...
function displayProducts(products) {
    ...
    products.forEach(p => {
        ...
        card.innerHTML = `
            <img src="${p.imagine_url}" alt="${p.nume}" crossorigin="anonymous">
            <div class="card-body">
                <div class="card-cat">${p.nume_categorie || 'General'}</div>
                <div class="card-name">${p.nume}</div>
                <div class="card-price">${p.pret} RON</div>
                <div style="font-size: 0.8rem; color: #888;">${p.stoc} in stock</div>
            </div>
        `;
...
function filterCategory(category, btn) {
    // 1. Update UI (bold the active button)
    ...
    if (category === 'All') {
        ...
        const filtered = allProducts.filter(p => p.nume_categorie === category);
